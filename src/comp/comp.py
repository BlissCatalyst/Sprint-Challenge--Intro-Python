...
char_range = [chr(i) for i in range(ord('C'), ord('G') + 1)]
c = [i.name for i in humans for j in char_range if i.name.startswith(j)]
print(c)
...
f = [tuple((i.name, i.age))
     for i in humans for j in range(27, 32 + 1) if i.age == j]
print(f)
